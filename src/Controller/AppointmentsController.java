...
public class AppointmentsController implements Initializable {
     ...
     * @param resourceBundle takes a resourceBundle as a parameter
     */
    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        appointmentsTableView.setOnMouseClicked(new EventHandler<MouseEvent>() {
            @Override
            public void handle(MouseEvent mouseEvent) {
                if (mouseEvent.getClickCount() == 1) {
                    ...
                    userIdTextField.setText(Integer.toString(selectedAppointment.getUserID()));
                    // Set start date and time picker values
                    // Get the UTC timestamp from the selected appointment
                    // Set the value of the start date and time picker controls
                    Timestamp utcStartTimestamp = selectedAppointment.getStartDateTime();
                    Instant startInstant = utcStartTimestamp.toInstant();
                    LocalDateTime startDateTime = LocalDateTime.ofInstant(startInstant, ZoneId.of("UTC"));
...
// Set the value of the end date and time picker controls
                    Timestamp utcEndTimestamp = selectedAppointment.getEndDateTime();
                    Instant endInstant = utcEndTimestamp.toInstant();
                    LocalDateTime endDateTime = LocalDateTime.ofInstant(endInstant, ZoneId.of("UTC"));
    ...
    public void addAppointment() {
        try (
            ...
            LocalTime startTime = LocalTime.parse((CharSequence) startTimePicker.getSelectionModel().getSelectedItem(), DateTimeFormatter.ofPattern("hh:mm a"));
     ...
     */
    public void populateTimePicker(ComboBox<String> timePicker) {
...
// Ask the user for confirmation
    ...
    private void updateAppointment() {
        ...
        selectedAppointment.setContactID(contactComboBox.getSelectionModel().getSelectedIndex() + 1);
        selectedAppointment.setType(typeComboBox.getSelectionModel().getSelectedItem());
        selectedAppointment.setCustomerID(Integer.parseInt(customerIdTextField.getText()));
        selectedAppointment.setUserID(Integer.parseInt(userIdTextField.getText()));

        LocalDate startDate = startDatePicker.getValue();
        LocalTime startTime = (LocalTime) startTimePicker.getSelectionModel().getSelectedItem();
        ...
        LocalDate endDate = endDatePicker.getValue();
        LocalTime endTime = (LocalTime) endTimePicker.getSelectionModel().getSelectedItem();
        LocalDateTime endDateTime = endDate.atTime(endTime);


// Update the appointment in the database
        try {
            PreparedStatement stmt = connection.prepareStatement("UPDATE appointments SET Title = ?, Description = ?, Location = ?, Contact_ID = ?, Type = ?, Start = ?, End = ?, Customer_ID = ?, User_ID = ? WHERE Appointment_ID = ?");
            stmt.setString(1, selectedAppointment.getTitle());
            stmt.setString(2, selectedAppointment.getDescription());
            stmt.setString(3, selectedAppointment.getLocation());
            stmt.setInt(4, selectedAppointment.getContactID());
            stmt.setString(5, selectedAppointment.getType());
            stmt.setTimestamp(6, Timestamp.valueOf(startDateTime));
            stmt.setTimestamp(7, Timestamp.valueOf(endDateTime));
            stmt.setInt(8, selectedAppointment.getCustomerID());
            stmt.setInt(9, selectedAppointment.getUserID());
    ...
    public void backToCustomers(ActionEvent actionEvent) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("../View/customerRecordForm.fxml"));
            Parent root = loader.load();
            Scene scene = new Scene(root);
            Stage stage = (Stage) ((Node) actionEvent.getSource()).getScene().getWindow();
            stage.setScene(scene);
