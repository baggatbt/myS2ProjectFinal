...
public class CustomerController implements Initializable {
        ...
        customerTable.setOnMouseClicked(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent mouseEvent) {
                if (mouseEvent.getClickCount() == 1) {
                    ...
                    firstLevelDivisionComboBox.getSelectionModel().select(selectedCustomer.getFirstLevelDivision());
                    try {
                        PreparedStatement countryStatement = JDBC.getConnection().prepareStatement("SELECT Country_ID FROM first_level_divisions WHERE Division_ID = ?");
                        countryStatement.setString(1,selectedCustomer.getFirstLevelDivision());
                        ResultSet countryResult = countryStatement.executeQuery();
                        countryResult.next();
                        int countryID = countryResult.getInt("Country_ID");
                        countryComboBox.getSelectionModel().select(countryID);
    ...
    public void updateCustomer() {
            try {
                ...
                stmt.setString(3, postalCodeField.getText());
                stmt.setString(4, phoneField.getText());
                // Convert the divisionName to divisionID
                String divisionName = firstLevelDivisionComboBox.getSelectionModel().getSelectedItem();
                int countryID = getCountryID();
                int divisionID = getDivisionID(countryID, divisionName);
                ...
                confirmation.setHeaderText("Confirm Update for Customer " + selectedCustomer.getCustomerID());
        ...
        private int getCountryID() throws SQLException {
            ...
            PreparedStatement statement = JDBC.getConnection().prepareStatement("SELECT Country_ID FROM countries WHERE Country = ?");
        ...
        private int getDivisionID(int countryID, String divisionName) throws SQLException {
            PreparedStatement statement = JDBC.getConnection().prepareStatement("SELECT Division_ID FROM first_level_divisions WHERE country_id = ? AND Division = ?");
   ...
   public void showDeleteMessage() {
    ...
    public void populateFirstLevelDivisionComboBox(String selectedCountry) {
        ...
        try {
            ...
            int countryID = -1;
            if(resultSet1.next()){
                countryID = resultSet1.getInt("Country_ID");
            }else {
                System.out.println("No countryPopBox");
            }
